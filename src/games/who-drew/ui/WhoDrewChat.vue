...
import WhoDrewChatMessage from './WhoDrewChatMessage.vue'
import WhoDrewChatSystem from './WhoDrewChatSystem.vue'
  ...
  <aside class="flex w-80 shrink-0 flex-col border-l border-border bg-bg-card">
    ...
    <ul class="flex-1 space-y-2 overflow-y-auto p-3">
      <li v-for="msg in game.chat" :key="msg.key">
        <WhoDrewChatSystem v-if="msg.system" :text="msg.text" />
        <WhoDrewChatMessage
          v-else
