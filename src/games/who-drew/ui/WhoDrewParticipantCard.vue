...
<template>
  ...
  >
    <span
      ...
      :class="participant.isTurn ? 'bg-brand text-on-brand' : 'bg-bg-elevated text-text-secondary'"
    >
      {{ participant.isTurn ? '🖌️' : '🙂' }}
    </span>
    ...
    <div class="min-w-0 flex-1">
      ...
      <p class="text-xs text-text-secondary">
        {{ participant.isTurn ? '그리는 중…' : participant.isPlaying ? '참가' : '관전' }}
      </p>
    </div>
  </li>
</template>
